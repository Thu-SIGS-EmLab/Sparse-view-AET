...
def Ap(angles, projections, iterations):  # Calculate the matrix of inverse Radon transform based on RESIRE
    ...
    # Apinv = Apinv.cpu().numpy()
    # Apinv=Apinv/np.max(Apinv)*255
    # cv2.imwrite("Apy.png",Apinv)
    return Apinv
...
class Diffusion(object):
    ...
    def run(self, model):
        ...
        gaussian_map = get_gaussian((256, 256, 8))
                ...
                for i, j in tqdm(time_pairs):
                    if j > -10:
                        ...
                        if j < i:
                            ...
                            Ax0_t = torch.sparse.mm(A, (x0_t).squeeze().permute(1, 2, 0).reshape(256 * 256, 8))
